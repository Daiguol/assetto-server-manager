...
export function EntryPoint() {
  ...
  $document
    .find(
      "input[type='checkbox']:not(input[name='EntryList.OverwriteAllEvents']:hidden):not(input[name='session-start-after-parent']:hidden)",
    )
    .bootstrapSwitch();
  ...
  $document.find("#open-in-simres").each(function (index, elem) {
    ...
    let link =
      window.location.href
        .replace(query, "")
        .split("#")[0]
        .replace("results", "results/download") +
      ".json" +
      query;
  ...
  $document.find("#simres-group").each(function (index, elem) {
    let query = window.location.search;
    let link =
      window.location.href.replace(query, "") + "/export-results" + query;
    ...
    $(this)
      ...
      .each(function () {
        ...
        if ($checkbox.is(":checked")) {
          ...
          $checkbox
            .after()
            .append($checkbox.clone().attr({ type: "hidden", value: 0 }));
...
function prettifyName(name, acronyms) {
  try {
    ...
    for (let i = 0; i < parts.length; i++) {
      if (
        (acronyms && parts[i].length <= 3) ||
        (acronyms && parts[i].match(nameRegex))
      ) {
        ...
        parts[i] = split
          .map((w) =>
            w.length > 0 ? w[0].toUpperCase() + w.substr(1).toLowerCase() : "",
          )
          .join(" ");
      }
    }
...
class RaceSetup {
  ...
  constructor($parent) {
    ...
    this.$trackLayoutDropdownParent =
      this.$trackLayoutDropdown.closest(".form-group");
  ...
  showEnabledSessions() {
    ...
    let $hiddenWhenBookingEnabled = that.$parent.find(
      ".hidden-booking-enabled",
    );
    let $visibleWhenBookingEnabled = that.$parent.find(
      ".visible-booking-enabled",
    );
    ...
    if (!$("#race-weekend-session").length) {
      ...
      $(".session-enabler").each(function (index, elem) {
        $(elem).on("switchChange.bootstrapSwitch", function (event, state) {
          ...
          let $panelLabel = that.$parent.find(
  ...
  toggleOverridePassword() {
    $("#OverridePassword").on(
      ...
      function (event, state) {
        let $this = $(this);
        let $replacementPasswordElem = $this
          .closest(".card-body")
          .find("#ReplacementPasswordWrapper");
  ...
  showSolWeathers(state) {
    this.$parent
      .find(".weather-graphics")
      .each(function (graphicsIndex, graphicsElement) {
    ...
    try {
      ...
      for (let index = 0; index < cars.length; index++) {
        ...
        for (let tyre in carTyres) {
          ...
          let $dropdownTyre = this.$tyresDropdown.find(
            "option[value='" + escapedTyre + "']",
          );
  ...
  showTrackDetails() {
    ...
    $.getJSON(jsonURL, function (trackInfo) {
      ...
      if (
        (overrideAmount && trackInfo.pitboxes <= overrideAmount) ||
        !overrideAmount
      ) {
  ...
  loadTrackLayouts() {
    ...
    if (
      availableLayouts &&
      !(
        availableLayoutKeys.length === 1 &&
        availableLayoutKeys[0] === "<default>"
      )
    ) {
      for (const [layout, name] of Object.entries(availableLayouts)) {
        this.$trackLayoutDropdown.append(
          this.buildTrackLayoutOption(layout, name),
        );
      }
  ...
  autoCompleteDrivers() {
    ...
    for (let entrant of possibleEntrants) {
      this.$parent
        .find(".entrant-autofill")
        .append($("<option>").val(entrant.Name).text(entrant.Name));
    }
  ...
  initEntrantsList() {
    ...
    function onEntryListCarChange() {
      ...
      showEntrantSkin(
        val,
        $this.closest(".entrant").find(".entryListSkin").val(),
        $this,
      );
    }
    ...
    function showEntrantSkin(currentCar, skin, $this) {
      ...
      if (
        currentCar in availableCars &&
        availableCars[currentCar] != null &&
        availableCars[currentCar].length > 0
      ) {
        if (skin === "random_skin") {
    ...
    this.$parent.find(".entryListSkin").change(function () {
      showEntrantSkin(
        $(this).closest(".entrant").find(".entryListCar").val(),
        $(this).val(),
        $(this),
      );
    });
    ...
    function populateEntryListSkinsAndSetups($elem, car) {
      ...
      $("<option value='random_skin'>&lt;random skin&gt;</option>").appendTo(
        $skinsDropdown,
      );
    ...
    function populateEntryListCars() {
      ...
      that.$parent.find(".entryListCar").each(function (index, val) {
        ...
        let $anyCar = $(
          "<option value='any_car_model'>Any Available Car</option>",
        );
        ...
        for (let val of cars.values()) {
          ...
          let realCarName = that.$carsDropdown
    ...
    that.$parent.find(".addEntrant").click(function (e) {
      ...
      for (let i = 0; i < numEntrantsToAdd; i++) {
        ...
        if (chosenCar) {
          ...
          $elem.find("[name='EntryList.Car']").append(
            $("<option>", {
              value: chosenCar,
        ...
        if (chosenSkin) {
          $elem.find("[name='EntryList.Skin']").append(
            $("<option>", {
              value: chosenSkin,
        ...
        if (fixedSetup) {
          $elem.find("[name='EntryList.FixedSetup']").append(
            $("<option>", {
              value: fixedSetup,
              text: fixedSetup,
              selected: true,
...
function handleWeatherFilesLoop(fileList) {
  ...
  for (let x = 0; x < fileList.length; x++) {
    ...
    if (
      fileList[x].name === "weather.ini" ||
      fileList[x].name.startsWith("preview.")
    ) {
  ...
  for (let x = 0; x < filesToUploadLocal.length; x++) {
    ...
    if (filesToUploadLocal[x].name.startsWith("preview.") && !previewDone) {
      ...
      reader.addEventListener("load", function () {
        $row.append(
          buildInfoPanel(reader.result.toString(), "Weather Preview"),
        );
      });
    }
  }
  ...
  if (filesToUploadLocal.length === 0) {
    $uploadButton.text(
      "Sorry, the files you uploaded don't seem to be a compatible weather preset!",
    );
    $uploadButton.empty();
  } else {
...
function handleCarFilesLoop(fileList) {
  ...
  for (let x = 0; x < fileList.length; x++) {
    // Find the files that the server is interested in
    if (
      fileList[x].name === "data.acd" ||
      fileList[x].name === "tyres.ini" ||
      fileList[x].name === "ui_car.json" ||
      fileList[x].name.startsWith("livery.") ||
      fileList[x].name.startsWith("preview.") ||
      fileList[x].name === "ui_skin.json" ||
      fileList[x].filepath.indexOf("/data/") !== -1
    ) {
      filesToUploadLocal.push(fileList[x]);
  ...
  for (let x = 0; x < filesToUploadLocal.length; x++) {
    ...
    if (filesToUploadLocal[x].name.startsWith("preview.") && !previewDone) {
      ...
      reader.addEventListener("load", function () {
        $row.append(
          buildInfoPanel(reader.result.toString(), "Livery: " + skinName),
        );
      });
    }
  ...
  if (filesToUploadLocal.length === 0) {
    $uploadButton.text(
      "Sorry, the files you uploaded don't seem to be a compatible car!",
    );
    $uploadButton.empty();
  } else {
...
function handleTrackDropFiles(ev) {
  ...
  getFilesWebkitDataTransferItems(items).then((files) => {
...
function handleTrackFilesLoop(fileList) {
  ...
  for (let x = 0; x < fileList.length; x++) {
    ...
    if (
      (fileList[x].name.startsWith("models") &&
        fileList[x].name.endsWith(".ini")) ||
      fileList[x].name === "surfaces.ini" ||
      fileList[x].name === "drs_zones.ini" ||
      fileList[x].filepath.includes("/ui/") ||
      fileList[x].name === "map.png" ||
      fileList[x].name === "map.ini"
    ) {
      filesToUploadLocal.push(fileList[x]);
    }
    ...
    if (filesToUploadLocal[x].filepath.includes("/ui/")) {
      if (filesToUploadLocal[x].name === "preview.png") {
        ...
        reader.addEventListener("load", function () {
          ...
          $row.append(
            buildInfoPanel(layoutInfo.preview, "Layout: " + layoutName),
          );
        });
      }
      ...
      if (filesToUploadLocal[x].name === "ui_track.json" && !tableDone) {
        ...
        reader.addEventListener("load", function () {
          $trackPanel.append(
            buildHtmlTable([JSON.parse(reader.result.toString())]),
          );
        });
      }
    }
  ...
  if (filesToUploadLocal.length === 0) {
    $uploadButton.text(
      "Sorry, the files you uploaded don't seem to be a compatible track!",
    );
    $uploadButton.empty();
  } else {
...
let championships = {
  ...
  initOpenChampionshipWatcher: function () {
    ...
    this.setOpenChampionshipOptions(
      $openChampionshipSwitch.bootstrapSwitch("state"),
    );

    let that = this;

    $openChampionshipSwitch.on(
      "switchChange.bootstrapSwitch",
      function (event, state) {
  ...
  initConfigureSignUpForm: function () {
    ...
    $("#ChampionshipSignUpFormEnabled").on(
      "switchChange.bootstrapSwitch",
      function (event, state) {
        if (state) {
  ...
  initSummerNote: function () {
    ...
    let wrapper = new SummernoteWrapper(
      $summerNote,
      {
        placeholder:
          "You can use this text input to share links to tracks/cars or any other resources, outline" +
  ...
  initDisplayOrder: function () {
    ...
    $switchOrder.click(function (e) {
      ...
      for (let i = $events.length; i >= 0; i--) {
        ...
        $($events[i]).insertAfter(
          $championship.find(".championship-event").last(),
        );
      }
